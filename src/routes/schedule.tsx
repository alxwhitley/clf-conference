...
function SchedulePage() {
  ...
  const daySessions = sessions.filter(
    (s) => s.day === currentDay && s.tracks.includes(track),
  );
            ...
            <div className="flex md:flex-col gap-2 overflow-x-auto">
              {days.map((d) => (
                <button
                  ...
                  className={cn(
                    "min-h-[44px] px-4 text-left text-[11px] font-bold uppercase tracking-[0.22em] rounded-[2px] border transition-colors whitespace-nowrap",
                    currentDay === d
          ...
          <div>
            <div className="flex items-baseline justify-between mb-6 pb-4 border-b-2 border-dark">
              <h2 className="font-display text-4xl md:text-5xl tracking-wider text-dark">
                {dayLabels[currentDay].long}
              </h2>
              <div className="eyebrow !text-text-muted">{dayLabels[currentDay].date}</div>
            </div>
            ...
            <div className="space-y-3">
              ...
              {daySessions.length === 0 && (
                <p className="text-text-muted">No sessions for this track on this day.</p>
              )}
            </div>
          </div>
        </div>
